...
A creature's ability to avoid danger, whether through toughness, speed, or skill. Give average creatures 3 STA, give hardier ones 6 STA, and and give serious threats 10 or higher.
...
- If an ability can be meaningfully avoided or defended against through traditional means, it is an Attack and interacts with STA.
...
X STA, X Armor, X BOD, X AGL, X CTR, Attack (dX, special items, _qualities_)
...
6 STA, 2 Armor, 14 BOD, 6 AGL, 4 CTR, smash & roll (d10, _blast_)
...
- **Critical Damage**: Target is pulled into its rolling momentum, losing an additional 1 BOD per round until freed (BOD save to escape).
...
- Sometimes the mechanics of the original system simply _do not translate_. That's OK! Try to focus on what's most interesting about the creature, and write a "version" of their ability more appropriate to Hardware.
- Abilities can sometimes be made into attacks (with a damage die), leaving Critical Damage to reveal the real impact. 
- If the monster has an attack that asks the PCs to attempt to dodge or save against some ability, consider making the ability an out-of-combat "trap" that the PCs can trigger. Otherwise, make it succeed automatically. Combat is dangerous, and it is up to the Admin to properly telegraph danger before the fighting begins.
...
The easiest way to convert a monster from another system is to read the original stat block and any associated text, then write a few sentences about the creature in plain English. Finally, convert what you've written to the Hardware monster stat block.
...
- She doesn't have super high STA, and she's quite fast.
...
8 STA, 12 BOD, 14 AGL, 15 CTR, claws (d6+d6), bite (d8)
